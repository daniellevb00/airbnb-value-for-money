...
# Which attributes score highest in terms of price/quality ratio?
...
# Mila's case
setwd("~/Documents/airbnb-value-for-money/gen/data-preparation/data")
ams_complete <- read.csv("ams_complete.csv")
ams_complete1 <- ams_complete[2:75]

# Price/quality ratio --> rev_value
...
bnb_res_pq <-augment(bnb_lm_pq)
ggplot(bnb_res_pq,aes(.resid))+geom_histogram(aes(y=..density..),binwidth=5)+stat_function(fun=dnorm,args=list(mean=mean(bnb_res_pq$.resid),sd=sd(bnb_res_pq$.resid)),color='red',size=2) 

# Random observations
plot(bnb_lm_pq) #1st time we press enter, we get a scatter plot to show us whether the observations are random. 
...
autoplot(bnb_lm_pq, which=1:3, nrow=1, ncol=3) #from code snippet building blocks
...
leverage_influence<-bnb_lm_pq%>%augment()%>%
  select(rev_value, mean_review,bedrooms,beds, balcony, lake_access, waterfront, backyard, room_type, kitchen, wifi, bed_linens, tv, washer, heating, free_parking, breakfast, pool, superhost, greeting_host, crib, baby_safety_gates, fire_extinguisher, security_cameras, carbon_monoxide_alarm, leverage =.hat, cooks_dist = .cooksd)%>%
...
# stargazer(bnb_lm_pq, bnb_lm_cleaned, 
#          title = 'Figure 2: Value-for-Money ratio based on Price Determinants',
#          dep.var.caption = 'Value-for-Money ratio',
#          dep.var.labels= '',
...
bnb_lm_pq$coef[[2]] #to retrieve the estimate of the first variable (its effect on the DV)
